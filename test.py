...
def post_process(det, mat_, trainval, c=None, s=None, resolution=None):
    ...
    if len(cropped_preds) > 0:
        cropped_preds[:, :, :2] = utils.img.kpt_affine(
            cropped_preds[:, :, :2] * 4, mat)  # size 1x16x3

    preds = np.copy(cropped_preds)
    # for inverting predictions from input res on cropped to original image
    if trainval != 'cropped':
        for j in range(preds.shape[1]):
            preds[0, j, :2] = utils.img.transform(
                preds[0, j, :2], c, s, resolution, invert=1)
    return preds
...
def inference(img, func, config, c, s):
    ...
    det = tmp['det'][0, -1] + tmp['det'][1, -
                                         1, :, :, ::-1][ds.flipped_parts['mpii']]
    if det is None:
        return [], []
    det = det/2
...
def mpii_eval(pred, gt, normalizing, num_train, bound=0.5):
    ...
    for p, g, normalize in zip(pred, gt, normalizing):
        for j in range(g.shape[1]):
            ...
            error = np.linalg.norm(
                p[0]['keypoints'][j, :2]-g[0, j, :2]) / normalize
            if idx >= num_train:
                if bound > error:
                    correct['all']['total'] += 1
                    correct['all'][joint] += 1
    ...
    for k in correct:
        ...
        for key in correct[k]:
            print('Atlas PCK @,', bound, ',', key, ':',
                  round(correct[k][key] / max(count[k][key], 1), 3), ', count:', count[k][key])
            # print('Atlas PCK @,', bound, ',', key, ':', round(correct_train[k][key] / max(
            #     count_train[k][key], 1), 3), ', count:', count_train[k][key])
        print('\n')
...
def get_img(config, num_eval=2958, num_train=300):
    ...
    # Our Own Images! Woohoooo
    with open("test_annotations_face_pants.json", "r") as f:
    # with open("test_annotations_face.json", "r") as f:
    # with open("test_annotations_pants_face.json", "r") as f:
    # with open("test_annotations_divi_face.json", "r") as f:
        val_f = json.load(f)
        n = len(val_f['imgname'])
        tr3 = tqdm.tqdm(range(0, n), total=n)
        # validation
        for i in tr3:
            path_t = '%s/%s' % (ds.img_dir, val_f['imgname'][i])
            # img
            orig_img = cv2.imread(path_t)[:, :, ::-1]
            c = val_f['center'][i]
            s = val_f['scale'][i]
...
def main():
    ...
    for anns, img, c, s, n, path in get_img(config, num_eval, num_train):
        gts.append(anns)
        pred = do(img, c, s)
        results[path] = pred
        preds.append(pred)
    ...
    # print(results)
    # print(type(results))
    with open("predictions_face_pants.json", "w") as outfile:
        json.dump(results, outfile, cls=NumpyEncoder, indent=4)
